...
class CareerDocumentExtractor
{
    ...
    private function extractText(UploadedFile $file, string $ext): string
    {
        ...
        if ($ext === 'pdf') {
            ...
            preg_match_all('/\(([^()\\\\]{4,200})\)/', $binary, $matches);
            $extracted = trim(implode(' ', array_slice($matches[1] ?? [], 0, 120)));

            return $extracted !== ''
                ? $extracted
                : 'PDF adjunto. Incluye también un resumen en texto si el extracto automático es incompleto.';
        }
